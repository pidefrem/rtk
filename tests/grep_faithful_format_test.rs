...
fn lowercase_h_is_no_filename_not_help() {
    ...
    assert_eq_grep_with_and_without_n(&["-h", "apple", &f1, &f2]);
...
fn lowercase_h_in_cluster_suppresses_filename() {
    ...
    assert_eq_grep_with_and_without_n(&["-hi", "apple", &f1, &f2]);
}
...
fn escaped_pipe_dialect_matches_grep_n() {
    ...
    assert_eq_grep_with_and_without_n(&["-E", r"\| None", &f]); // ERE: literal pipe → only "| None"
    assert_eq_grep_with_and_without_n(&["-E", r"a\|b", &f]); // ERE: literal → only "a|b"
    assert_eq_grep_with_and_without_n(&[r"a\|b", &f]); // BRE: alternation → a, b, a|b
    assert_eq_grep_with_and_without_n(&["-F", "a|b", &f]); // fixed-string pipe → only "a|b"
}
