...
pub fn run(
    ...
    // would not have shown. GNU `-h`/`--no-filename` forces the filename off,
    // overriding every reason to show one, so it gates the whole expression.
    let no_filename =
        has_short_flag(&extra_args, 'h') || extra_args.iter().any(|f| f == "--no-filename");
    let show_file = !no_filename && (by_file.len() > 1 || show_file(&paths, &extra_args));
